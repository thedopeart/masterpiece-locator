/**
 * Fix broken Cleveland Museum images
 * 1. Restore original WikiArt URLs from the cleveland-found.json
 * 2. Search Wikimedia Commons for proper replacements
 * 3. Upload remaining to Supabase
 */
...
// Step 1: Restore original WikiArt URLs
async function restoreOriginalUrls() {
  console.log('=== Step 1: Restoring Original WikiArt URLs ===\n');

  const resultsFile = path.join(__dirname, 'results/cleveland-found.json');
...
async function main() {
  console.log('=== Fixing Cleveland Museum Images ===\n');

  // Step 1: Restore original URLs
  ...
  console.log('\n=== Complete ===');
  console.log(`Wikimedia: ${wikimediaFound.length}`);
  console.log(`Supabase: ${notFound.length - (notFound.length > 0 ? 0 : 0)}`);
